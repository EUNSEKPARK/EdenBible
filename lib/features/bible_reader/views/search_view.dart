...
/// 성경 전문 검색 화면
/// Navigator.pop으로 {'bookId': int, 'chapter': int} 반환
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        Container(padding: const EdgeInsets.fromLTRB(16, 8, 16, 12), child: Row(children: [
          ...
          Expanded(child: Container(height: 44, padding: const EdgeInsets.symmetric(horizontal: 14),
            decoration: BoxDecoration(color: isDark ? EdenColors.surfaceVariantDark : const Color(0xFFF0EEE8), borderRadius: BorderRadius.circular(22)),
            child: Row(children: [
        ...
        Expanded(child: _results.isEmpty && !_searching
          ...
          : _searching ? const Center(child: CircularProgressIndicator()) : _buildResults(isDark)),
      ])),
    );
  }
      ...
      Expanded(child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return _SearchResultTile(verse: verse, query: _lastQuery, isDark: isDark,
            onTap: () => Navigator.pop(context, {'bookId': verse.bookId, 'chapter': verse.chapter}));
        })),
    ]);
  }
...
class _SearchResultTile extends StatelessWidget {
  final BibleVerse verse; final String query; final bool isDark; final VoidCallback? onTap;
  const _SearchResultTile({required this.verse, required this.query, required this.isDark, this.onTap});
...
class _HighlightedText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    while (true) {
      ...
      if (index == -1) { spans.add(TextSpan(text: text.substring(start))); break; }
      if (index > start) spans.add(TextSpan(text: text.substring(start, index)));
      spans.add(TextSpan(text: text.substring(index, index + query.length),
