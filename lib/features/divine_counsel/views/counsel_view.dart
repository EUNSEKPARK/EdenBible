...
class _CounselViewState extends State<CounselView> {
  ...
  void _sendMessage(String text) async {
    ...
    if (response != null) {
      ...
      setState(() {
        _isTyping = false;
        _messages.add(_ChatMessage(isUser: false, text: '마음을 나눠주셔서 감사해요. 조금 더 자세히 이야기해 주시면 당신의 상황에 맞는 말씀을 찾아드릴게요.', verses: [], prayerGuide: null));
      });
    }
    _scrollToBottom();
  }
      ...
      children: [
        if (_messages.length <= 2)
          SingleChildScrollView(
            ...
            child: Row(
              children: _suggestions.take(4).map((q) => Padding(
          ...
          child: SafeArea(
            ...
            child: Row(children: [
              ...
              GestureDetector(
                ...
                child: Container(width: 44, height: 44, decoration: BoxDecoration(color: EdenColors.primary, shape: BoxShape.circle), child: const Icon(Icons.arrow_upward_rounded, color: Colors.white, size: 22)),
              ),
            ]),
          ),
        ),
...
class _UserBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Align(alignment: Alignment.centerRight, child: Container(
      ...
      decoration: BoxDecoration(color: EdenColors.primary, borderRadius: const BorderRadius.only(topLeft: Radius.circular(20), topRight: Radius.circular(20), bottomLeft: Radius.circular(20), bottomRight: Radius.circular(4))),
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Padding(padding: const EdgeInsets.only(bottom: 6, left: 4), child: Row(children: [
            Icon(Icons.auto_awesome, size: 14, color: EdenColors.accent),
              ...
              GestureDetector(
                onTap: () {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('대화 내용을 저장했습니다'), behavior: SnackBarBehavior.floating, duration: Duration(seconds: 2)),
                  );
                },
                child: Padding(padding: const EdgeInsets.all(4), child: Icon(Icons.bookmark_outline, size: 16, color: EdenColors.secondary)),
              ),
            ]),
          ),
        ]),
