...
class EdenTopAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  void _showMenuSheet(BuildContext context) {
    showModalBottomSheet(context: context,
      ...
      builder: (ctx) => SafeArea(child: Padding(padding: const EdgeInsets.symmetric(vertical: 24, horizontal: 20),
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('메뉴', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: EdenColors.primary)),
          ...
          ListTile(leading: Icon(Icons.nightlight_round, color: Colors.amber), title: const Text('수면 묵상'),
            subtitle: const Text('잔잔한 말씀과 함께 잠들기', style: TextStyle(fontSize: 12)),
            onTap: () { Navigator.pop(ctx); Navigator.push(context, MaterialPageRoute(builder: (_) => const SleepModeView())); }),
          ListTile(leading: Icon(Icons.school_rounded, color: EdenColors.secondary), title: const Text('성경 연구'),
            onTap: () { Navigator.pop(ctx); Navigator.push(context, MaterialPageRoute(builder: (_) => const StudyView())); }),
    ...
    showModalBottomSheet(context: context,
      ...
      builder: (ctx) => SafeArea(child: Padding(padding: const EdgeInsets.all(24),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 72, height: 72, decoration: BoxDecoration(shape: BoxShape.circle, color: EdenColors.primary.withValues(alpha: 0.15)),
            child: Icon(Icons.person_rounded, size: 36, color: EdenColors.primary)),
          const SizedBox(height: 16),
          const Text('사용자', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
          const SizedBox(height: 4),
          Text('에덴 성경책과 함께하는 중', style: TextStyle(fontSize: 13, color: EdenColors.textTertiaryLight)),
